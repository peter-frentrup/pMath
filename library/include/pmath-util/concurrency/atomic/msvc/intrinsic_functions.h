...
intptr_t pmath_atomic_read_aquire(pmath_atomic_t *atom) {
  ...
  pmath_atomic_barrier(); // all reads or writes below this line keep below it (pmath_atomic_barrier() even a full barrier)
  return data;
}
...
void pmath_atomic_write_release(pmath_atomic_t *atom, intptr_t value){
  pmath_atomic_barrier(); // all reads or writes above this line keep above it (pmath_atomic_barrier() even a full barrier)
  atom->_data = value;
}
