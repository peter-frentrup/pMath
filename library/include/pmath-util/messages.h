...
/**\defgroup messages Messages
   ...
   Example: The built-in Sin function invokes Sin::arg1 and returns unevaluated
            when it is called with a wrong number of arguments.
...
/**\brief Print a message with pMath object arguments.
   \param symbol    The symbol that defines the message. It wont be freed. 
                    PMATH_NULL is be treated as pmath_current_head().
   \param tag       The message's tag as a zero-terminated C string.
   \param argcount  The number of following arguments.
   \param ...       Exactly \a argcount pMath objects. They will all be freed.
   
   \note If symbol==PMATH_NULL and pmath_current_head() is an expression f(), 
   the function acts as if pmath_current_head() was f.
...
/**\brief Generate a General::arg* message (invalid argument count).
   ...
   One of the following messages may be generated:
    General::arg1  \n
    General::argxu \n
    General::argx  \n
    General::argtu \n
...
/**\brief Print a syntax warning or error message.
   ...
   \param filename           The file from where the input came or PMATH_NULL to 
                             omit it. It will be freed.
   \param lines_before_code  The number of lines in the input file before 
                             \a code was read. This is ignored if \a filename is 
                             PMATH_NULL.
   
   This function produces a Syntax::bgn, Syntax::bgnf, Syntax::nxt, 
   Syntax::nxtf, Syntax::more, Syntax::moref, Syntax::newl or Syntax::newlf 
   message, depending on where the syntax error is in the \a code and whether 
   \a filename is PMATH_NULL or not. It can be used to report errors/warnings 
   from pmath_spans_from_string().
 */
PMATH_API 
