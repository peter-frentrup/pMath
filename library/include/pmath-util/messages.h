...
/**\brief Print a message with pMath object arguments.
   \param symbol The symbol, that defines the message. It wont be freed. NULL
   ...
   \note If symbol==NULL and pmath_current_head() is an expression f(), the 
...
/**\brief Find a message's text.
   ...
   \return The message's content or NULL if nothing was found or the 
...
/**\brief Print a syntax warning or error message.
   ...
   \param filename The file from where the input came or NULL to omit it. It 
          will be freed.
   \param lines_before_code The number of lines in the input file before \a code
          was read. This is ignored if \a filename is NULL.
   ...
   NULL. It can be used to report errors/warnings from 
