...
/** \defgroup dynamics Dynamic Evaluation
    ...
    Unlike in the classical observer pattern, observation is one-shot: every interrested
    ...
    This behaviour is an explicit design goal: Let the dynamic object be some 
    ...
    if the symbol is not yet bound to id but may bind to it (local symbols created during 
    ...
    Dynamic objects are managed by the front-end. It does so by evaluating
    Internal`DynamicEvaluateMultiple(Dynamic(expr, ...), id) when a dynamic object 
    with given id is interrested in the value of expr and in changes to that value.
