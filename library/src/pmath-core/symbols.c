...
PMATH_API pmath_symbol_t pmath_symbol_get(
  ...
  if(pmath_string_length(name) == 1) {
    ...
    switch(ch) {
      ...
      case 0x2147:
        pmath_unref(name);
        return pmath_ref(pmath_System_ExponentialE);
  ...
  if(create) {
    ...
    new_symbol->name        = _pmath_string_set_debug_metadata(pmath_ref(name), PMATH_NULL);
...
PMATH_API pmath_symbol_t pmath_symbol_create_temporary(
  ...
  if(pmath_is_null(result)) {
    ...
    new_symbol->lock        = NULL;
    new_symbol->name        = _pmath_string_set_debug_metadata(name, PMATH_NULL);
