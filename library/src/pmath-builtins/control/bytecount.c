...
static size_t bytecount(
  ...
  switch(PMATH_AS_PTR(obj)->type_shift){
    case PMATH_TYPE_SHIFT_INTEGER:
      return abs(((struct _pmath_integer_t*)PMATH_AS_PTR(obj))->value->_mp_size) * sizeof(mp_limb_t)
           + sizeof(struct _pmath_integer_t);
    
    case PMATH_TYPE_SHIFT_QUOTIENT: 
      return bytecount((pmath_t)PMATH_FROM_PTR(((struct _pmath_quotient_t*)PMATH_AS_PTR(obj))->numerator))
           + bytecount((pmath_t)PMATH_FROM_PTR(((struct _pmath_quotient_t*)PMATH_AS_PTR(obj))->denominator))
           + sizeof(struct _pmath_quotient_t);
    
    case PMATH_TYPE_SHIFT_MP_FLOAT:
      return (((struct _pmath_mp_float_t*)PMATH_AS_PTR(obj))->value->_mpfr_prec 
           + 8 * sizeof(mp_limb_t) - 1) / 8
    ...
    case PMATH_TYPE_SHIFT_STRING:
      return LENGTH_TO_CAPACITY(((struct _pmath_string_t*)PMATH_AS_PTR(obj))->length) * sizeof(uint16_t)
           + STRING_HEADER_SIZE;
    ...
    case PMATH_TYPE_SHIFT_EXPRESSION_GENERAL_PART: {
      ...
      len = ((struct _pmath_unpacked_expr_t*)PMATH_AS_PTR(obj))->length;
