...
static void parallel_map_indexed(struct parallel_map_info_t *info) {
...
PMATH_PRIVATE pmath_t builtin_parallelmapindexed(pmath_expr_t expr) {
  /* ParallelMapIndexed(list, f, startlevel..endlevel)
     ParallelMapIndexed(list, f)    = ParallelMapIndexed(list, f, 1..1)
     ParallelMapIndexed(list, f, n) = ParallelMapIndexed(list, f, n..n)
   */
      ...
      if(info.items) {
        for(i = 0; i < (size_t)task_count; ++i) {
          tasks[i] = _pmath_task_new_with_thread(
                       (pmath_callback_t)parallel_map_indexed,
                       dummy,
                       &info,
                       NULL);
        }
