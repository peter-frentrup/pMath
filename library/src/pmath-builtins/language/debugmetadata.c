...
PMATH_PRIVATE pmath_t builtin_developer_getdebugmetadata(pmath_expr_t expr) {
  ...
  pmath_unref(expr);
  expr = pmath_get_debug_metadata(obj);
  pmath_unref(obj);
  return expr;
}

static pmath_t set_debug_metadata_at(
  ...
  if(i <= pmath_expr_length(indices)) {
        ...
        pmath_is_expr(obj))
    {
      ...
      sub = set_debug_metadata_at(sub, info, indices, i + 1);
      
      info = _pmath_expr_get_debug_metadata(obj);
      obj = pmath_expr_set_item(obj, (size_t)PMATH_AS_INT32(index), sub);
      obj = _pmath_expr_set_debug_metadata(obj, info);
      return obj;
    }
  ...
  return pmath_try_set_debug_metadata(obj, info);
}

PMATH_PRIVATE pmath_t builtin_developer_setdebugmetadataat(pmath_expr_t expr) {
  /* SetDebugMetadataAt(obj, info, [i1, i2, ...])
   */
  pmath_t obj, info;
  size_t exprlen = pmath_expr_length(expr);
  ...
  obj = set_debug_metadata_at(obj, info, expr, 3);
  pmath_unref(expr);
