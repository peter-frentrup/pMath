...
static pmath_bool_t try_tanh_of_exact_complex(pmath_t *expr, pmath_t x) {
  ...
  if(pmath_equals(re, PMATH_FROM_INT32(0))) {
    pmath_unref(re);
    pmath_unref(*expr);
    *expr = TIMES(COMPLEX(INT(0), INT(1)), FUNC(pmath_ref(PMATH_SYMBOL_TAN), im));
    return TRUE;
  }
...
PMATH_PRIVATE pmath_t builtin_tanh(pmath_expr_t expr) {
  ...
  x = pmath_expr_get_item(expr, 1);
  if(_pmath_complex_try_evaluate_acb(&expr, x, acb_tanh)) { // TODO: return CINFTY when applicable
    pmath_unref(x);
    return expr;
  }
