...
PMATH_PRIVATE pmath_t builtin_sin(pmath_expr_t expr){
  ...
  if(pmath_instance_of(x, PMATH_TYPE_MP_FLOAT)){
    ...
    if(tmp){
      ...
      sin_val = mpfr_get_d(tmp->error, GMP_RNDN);
      ...
      if(result){
        mpfr_sin(result->value, ((struct _pmath_mp_float_t*)PMATH_AS_PTR(x))->value, GMP_RNDN);
        ...
        if(pmath_same(snd, PMATH_SYMBOL_PI)){
          ...
          && pmath_integer_fits_ui((pmath_integer_t)PMATH_FROM_PTR(((struct _pmath_quotient_t*)PMATH_AS_PTR(fst))->denominator))
            ...
            if(num <= den / 2)
              switch(den){
                ...
                case 12:
                  ...
                  if(num == 1)
                    return DIV(PLUS(INT(-1), SQRT(INT(3))), TIMES(INT(2), SQRT(INT(2))));
            ...
            if(pmath_same(cmp, PMATH_SYMBOL_TRUE)){ // 1/2 Pi <= x < Pi
              ...
              return pmath_expr_set_item(expr, 1, x); // return Sin((1-fst)*PI)
            }
            ...
            if(pmath_same(cmp, PMATH_SYMBOL_TRUE)){ // Pi <= x < 2 Pi
              ...
              x = pmath_expr_set_item(x, 1, fst);
              return NEG(pmath_expr_set_item(expr, 1, x)); // return -Sin((2-fst)*PI)
            }
            ...
            x    = pmath_expr_set_item(x,    1, fst);
            return pmath_expr_set_item(expr, 1, x); // return Sin((fst - Floor(fst, 2))*PI)
          }
        }
      }
