...
static pmath_t get_default_options(pmath_t head) { // fn wont be freed
  ...
  if(rules) {
    pmath_t result = pmath_expr_new_extended(
                       pmath_ref(PMATH_SYMBOL_OPTIONS), 1, head);
...
static pmath_bool_t verify_subset_is_options_subset(
  ...
  if(pmath_same(head, PMATH_SYMBOL_UNDEFINED))
    return TRUE;
...
PMATH_API pmath_t pmath_option_value(
  ...
  fnoptions = get_default_options(fn);
