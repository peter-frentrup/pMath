...
#if PMATH_BYTE_ORDER == -1 /* little endian */
#  define blk0(i) \
    (block->l[i] = \
...
void _pmath_sha1_update(struct _pmath_sha1_context_t *context, const uint8_t *data, uint32_t len) {
  ...
  if((j + len) > 63) {
    ...
    _pmath_sha1_transform(context->state, context->buffer);
    
    for (; i + 63 < len; i += 64) {
      _pmath_sha1_transform(context->state, &data[i]);
    }
...
void _pmath_sha1_final(struct _pmath_sha1_digest_t hash_out[1], struct _pmath_sha1_context_t *context) {
  ...
  c = 0200;
  _pmath_sha1_update(context, &c, 1);
  while((context->count[0] & 504) != 448) {
    c = 0000;
    _pmath_sha1_update(context, &c, 1);
  }
  
  _pmath_sha1_update(context, finalcount, 8); /* Should cause a _pmath_sha1_transform() */
  for(i = 0; i < 20; i++) {
    hash_out->digest[i] = 
...
#define BLOCK_SIZE   0x1000
  ...
  while(len > BLOCK_SIZE) {
    _pmath_sha1_update(&ctx, buf, BLOCK_SIZE);
    len-= BLOCK_SIZE;
    buf+= BLOCK_SIZE;
  ...
  _pmath_sha1_update(&ctx, buf, (uint32_t)len);
  
//FINISH:
  _pmath_sha1_final(hash_out, &ctx);
  
  if(pmath_aborting()) 
    memset(hash_out->digest, 0, sizeof(hash_out->digest));
