...
$moduleCacheDirectory = (Join-Path $scriptPath "dependencies\windows")
...
Else {
	Write-Verbose "Cache file $moduleCacheFile not found. Rebuilting..."
...
Copy-Item -Path (Join-Path $scriptPath "dependencies\default\*") -Destination $targetDirectory -Force
