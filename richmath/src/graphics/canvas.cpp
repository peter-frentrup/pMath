...
void Canvas::bitop_fill(BitOperations op, int pen_color){
  if(show_only_text)
    return;
    
  bitop_fill_preserve(op, pen_color);
  new_path();
}

  static uint32_t doBitOp0(   uint32_t p, uint32_t d){ return 0; }
  static uint32_t doBitOpDPon(uint32_t p, uint32_t d){ return ~(d | p); }
  static uint32_t doBitOpDPna(uint32_t p, uint32_t d){ return d & (~p); }
  static uint32_t doBitOpPn(  uint32_t p, uint32_t d){ return ~p; }
  static uint32_t doBitOpPDna(uint32_t p, uint32_t d){ return p & (~d); }
  static uint32_t doBitOpDn(  uint32_t p, uint32_t d){ return ~d; }
  static uint32_t doBitOpDPx( uint32_t p, uint32_t d){ return d ^ p; }
  static uint32_t doBitOpDPan(uint32_t p, uint32_t d){ return ~(d & p); }
  static uint32_t doBitOpDPa( uint32_t p, uint32_t d){ return d & p; }
  static uint32_t doBitOpDPxn(uint32_t p, uint32_t d){ return ~(d ^ p); }
  static uint32_t doBitOpD(   uint32_t p, uint32_t d){ return d; }
  static uint32_t doBitOpDPno(uint32_t p, uint32_t d){ return d | (~p); }
  static uint32_t doBitOpP(   uint32_t p, uint32_t d){ return p; }
  static uint32_t doBitOpPDno(uint32_t p, uint32_t d){ return p | (~d); }
  static uint32_t doBitOpDPo( uint32_t p, uint32_t d){ return d | p; }
  static uint32_t doBitOp1(   uint32_t p, uint32_t d){ return 1; }
  
  static uint32_t (*bitop_functions[16])(uint32_t,uint32_t) = {
    doBitOp0,
    doBitOpDPon,
    doBitOpDPna,
    doBitOpPn,
...
void Canvas::bitop_fill_preserve(BitOperations op, int pen_color){
  if(show_only_text)
    return;
  ...
  switch(cairo_surface_get_type(target)){
    case CAIRO_SURFACE_TYPE_IMAGE: {
      ...
      cairo_path_extents(_cr, &dx1, &dy1, &dx2, &dy2);
      
      double cx1, cx2, cy1, cy2;
      cairo_clip_extents(_cr, &cx1, &cy1, &cx2, &cy2);
      ...
      cairo_bool_t (*test)(cairo_t*,double,double) = cairo_in_fill;
      cairo_path_t *path = cairo_copy_path(_cr);
      int num_points = 0;
      int end = 0;
      ...
      uint32_t (*f)(uint32_t,uint32_t) = bitop_functions[op];
      ...
      switch(format){
        ...
        case CAIRO_FORMAT_RGB24: {
          ...
          for(int y = y1;y <= y2;++y){
            for(int x = x1;x <= x2;++x){
              if(test(_cr, x + 0.5, y + 0.5)){
                uint32_t c = *(uint32_t*)&data[x * 4];
                
                //c = (c & 0xFF000000U) | ((~c) & 0xFFFFFF);
                
                *(uint32_t*)&data[x * 4] = 
                  (c & 0xFF000000U) | (f(pen_color, data[x * 4]) & 0x00FFFFFFU);
              }
            }
    ...
    case CAIRO_SURFACE_TYPE_WIN32: {
      HDC hdc = cairo_win32_surface_get_dc(target);
      int old_rop2 = GetROP2(hdc);
      SetROP2(hdc, 1 + op);
      HPEN pen = CreatePen(PS_NULL, 0, 0); // PS_SOLID
      HBRUSH brush = CreateSolidBrush(
          ((pen_color & 0xFF0000) >> 16)
        |  (pen_color & 0x00FF00)
        | ((pen_color & 0x0000FF) << 16));//GetSysColorBrush(COLOR_HIGHLIGHT);
      
      double cx1, cy1, cx2, cy2;
      cairo_clip_extents(_cr, &cx1, &cy1, &cx2, &cy2);
      SelectClipRgn(hdc, NULL);
      IntersectClipRect(hdc, floor(cx1), floor(cy1), ceil(cx2), ceil(cy2));
      
      cairo_path_t *path = cairo_copy_path_flat(_cr);
      
      int start = 0;
      while(start < path->num_data){
        int num_points = 0;
        int end = start;
        while(end < path->num_data
        && path->data[end].header.type != CAIRO_PATH_CLOSE_PATH){
        ...
        if(num_points > 2){
          POINT *points = (POINT*)malloc((1 + num_points) * sizeof(POINT));
          if(points){
            int i = start;
            int j = 0;
            float x, y;
            
            while(i < end - 1 && j < num_points){
              x = path->data[i+1].point.x;
              y = path->data[i+1].point.y;
              
              user_to_device(&x, &y);
              
              points[j].x = (long)floor(x + 0.5);
              points[j].y = (long)floor(y + 0.5);
            ...
            Polygon(hdc, points, num_points + 1);
