...
SharedPtr<OTMathShaper> OTMathShaperDB::find(String name, FontStyle style){
  ...
  db->name = name;
  db->shapers[style] = new OTMathShaper(db, style);
  db->fi = &db->shapers[style]->fi;
...
//      pmath_debug_print("Asana Math degree glyph: %d\n", db->fi->char_to_glyph(0x00B0));
    ...
    if(db->fi->char_to_glyph(PMATH_CHAR_ASSIGNDELAYED) == 0){ // ::= (its needed, but not in Cambria Math)
      ...
      static_canvas.canvas->set_font_face(db->shapers[style]->font(0));
...
SharedPtr<OTMathShaper> OTMathShaperDB::find(FontStyle style){
  if(shapers[style].is_valid())
    return shapers[style];
  
  ref();
  shapers[style] = new OTMathShaper(this, style);
