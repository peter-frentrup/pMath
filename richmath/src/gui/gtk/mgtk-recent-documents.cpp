...
namespace richmath {
  class MathGtkRecentInfoMenus {
    public:
      Hashset<String> ignore_files;
      
      Expr recent_info_to_menu_item(int index, GtkRecentInfo *info);
  };
}
...
Expr MathGtkRecentDocuments::as_menu_list() {
  ...
  for(GList *cur = items; cur != nullptr && count < 9; cur = cur->next) {
    Expr menu_item = m.recent_info_to_menu_item(count, (GtkRecentInfo*)cur->data);
...
Expr MathGtkRecentInfoMenus::recent_info_to_menu_item(int index, GtkRecentInfo *info) {
